...
public class DepartamentoController {
    ...
    public ResponseEntity<List<DepartamentoDto>> getAll() {
        List<DepartamentoDto> departamentos = service.getAll();
        return ResponseEntity.ok(departamentos);
    }
    // OBTENER POR ID
    @GetMapping("/{id}")
    public ResponseEntity<DepartamentoDto> getById(@PathVariable Long id) {
        Optional<DepartamentoDto> departamentoDto = service.getById(id);
    ...
    public ResponseEntity<DepartamentoDto> save(@RequestBody DepartamentoDto dto) {
        DepartamentoDto created = service.save(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }
    // ACTUALIZAR DEPARTAMENTO
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('AdminGeneral')")
    public ResponseEntity<Void> update(@PathVariable Long id, @RequestBody DepartamentoDto dto) {
